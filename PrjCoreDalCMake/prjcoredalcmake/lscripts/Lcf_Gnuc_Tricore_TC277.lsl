...
SECTIONS
{
    ...
    .multiprogram_data (0x5001DC00) (NOLOAD) :  FLAGS(awlc0) /*1KB max*/
    {
        PROVIDE(multiprogram_data_begin = .);
        *(SORT(multiprogram_data*))
        . = ALIGN(8);
        PROVIDE(multiprogram_data_end = .);
    } > dsram2

	.calrom (0x80018000) : FLAGS(arl)
	{
		_calromStart = .;
		KEEP (*(.calrom));
		. = _calromStart + (32K);
	...
	.traptab_tc2 ALIGN(8)/*(LCF_TRAPVEC2_START)*/ :
	{
		PROVIDE(__TRAPTAB_CPU2 = .);
		KEEP (*(.traptab_cpu2));
	} > pfls0
	
	.traptab_tc1 ALIGN(8)/*(LCF_TRAPVEC1_START)*/ :
	{
