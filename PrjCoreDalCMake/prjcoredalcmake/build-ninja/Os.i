# 1 "D:/PrjCoreDalCMake/prjcoredalcmake/deps/core/Os/Os.c"
...
# 2 "D:/PrjCoreDalCMake/prjcoredalcmake/deps/core/Os/Os.c" 2


extern Os_ConfigType Os_Config;


MessageBufferHandle_t xControlMessageBuffer[2][ 2 ];
StaticStreamBuffer_t xControlMessageBufferStruct[ 2 ][ 2 -1 ];
uint8_t ucControlBufferStorage[2][ 2 -1][ 60 ];
...
static void OsInitConfiguration(void){
    BaseType_t curCoreId = ((BaseType_t)IfxCpu_getCoreId());
 for (int i = 0; i<Os_Config.tasksNum; i++){
    ...
    if (curCoreId == 0)
    {
        for(int i = 0; i < 2; i++)
     {
            ...
            for(int j = 0; j < 2; j++){
                if (i != j){
                    xControlMessageBuffer[i][j] = ( MessageBufferHandle_t ) xStreamBufferGenericCreateStatic( 60, 0, ( ( BaseType_t ) 1 ), &ucControlBufferStorage[i][k][0], &xControlMessageBufferStruct[i][k] )
     ...
     {
            Os_Config.multicoreMsgs[i].dataMsgBufferHandle = ( MessageBufferHandle_t ) xStreamBufferGenericCreateStatic( Os_Config.multicoreMsgs[i].dataMsgBufferStorageSize, 0, ( ( BaseType_t ) 1 ), Os_Config.multicoreMsgs[i].dataMsgBufferStorage, Os_Config.multicoreMsgs[i].dataMsgBufferStruct )
...
void SetEvent(TaskType TaskID, EventMaskType Mask)
{
    xTaskGenericNotify( ( Os_Config.tasks[TaskID].taskHandle ), ( 0 ), ( (uint32_t)Mask ), ( eSetBits ), ((void *)0) )
...
void vGenerateCore2CoreInterruptNotif( void * xUpdatedMessageBuffer )
{
    ...
    if (msgIsFound)
    {
     ...
     {
            while( xStreamBufferSend( ( StreamBufferHandle_t ) xControlMessageBuffer[rxCoreId][txCoreId], &xUpdatedBuffer, sizeof( xUpdatedBuffer ), 0 ) != sizeof( xUpdatedBuffer ) )
            {

            }

            vPortGenerateCore2CoreInterrupt( rxCoreId, 0 );
     }
    }
}
void vGenerateCore2CoreInterruptConfirm( void * xUpdatedMessageBuffer )
{
    ...
    if (msgIsFound)
    {
        ...
        if( xUpdatedBuffer != xControlMessageBuffer[rxCoreId][txCoreId] )
     {
            ...
            vPortGenerateCore2CoreInterrupt( txCoreId, 1 );
     }
    }
}
void vCore2CoreInterruptHandlerNotif(uint8_t srcCoreId)
{
 ...
 while( xStreamBufferReceiveFromISR( ( StreamBufferHandle_t ) xControlMessageBuffer[ ((BaseType_t)IfxCpu_getCoreId()) ][srcCoreId], &xUpdatedMessageBuffer, sizeof( xUpdatedMessageBuffer ), &xHigherPriorityTaskWoken )
                                       ...
                                       == sizeof( xUpdatedMessageBuffer ) )
 {
  ...
  xStreamBufferSendCompletedFromISR( ( StreamBufferHandle_t ) xUpdatedMessageBuffer, &xHigherPriorityTaskWoken );
 }
 ...
 if( xHigherPriorityTaskWoken != ( ( BaseType_t ) 0 ) ) { __asm__ volatile ("syscall ""0" ::: "memory"); };
}
void vCore2CoreInterruptHandlerConfirm(void)
{
    ...
    for (int i = 0; i < Os_Config.multicoreMsgsNum; i++ )
    {
        ...
        {
           xStreamBufferReceiveCompletedFromISR( ( StreamBufferHandle_t ) Os_Config.multicoreMsgs[i].dataMsgBufferHandle, &xHigherPriorityTaskWoken );
        }
    }
...
static StaticTask_t xIdleTaskTCB[2];
static StackType_t uxIdleTaskStack[2][ ( ( unsigned short ) 128 ) ];



    *ppxIdleTaskTCBBuffer = &xIdleTaskTCB[ ((BaseType_t)IfxCpu_getCoreId()) ];


    *ppxIdleTaskStackBuffer = uxIdleTaskStack[ ((BaseType_t)IfxCpu_getCoreId()) ];
...
static StaticTask_t xTimerTaskTCB[2];
static StackType_t uxTimerTaskStack[2][ ( ( unsigned short ) 128 ) ];



    *ppxTimerTaskTCBBuffer = &xTimerTaskTCB[ ((BaseType_t)IfxCpu_getCoreId()) ];


    *ppxTimerTaskStackBuffer = uxTimerTaskStack[ ((BaseType_t)IfxCpu_getCoreId()) ];
